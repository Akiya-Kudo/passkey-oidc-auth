...
export function registerInteractionRoutes(router: Router, provider: Provider) {
	router.get(`${oidcConfig.routes.interaction}/:uid`, async (ctx) => {
	...
	router.post(`${oidcConfig.routes.interaction}/:uid/login`, async (ctx) => {
	...
	router.post(`${oidcConfig.routes.interaction}/:uid/confirm`, async (ctx) => {
		// TODO: consent 承認処理
		ctx.status = 501;
		ctx.body = {
			error: "not_implemented",
